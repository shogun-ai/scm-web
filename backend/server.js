...
const AI_LOAN_OFFICER_VERSION = '2026-05-27';
...
function buildRuleBasedLoanOfficerAssessment(loanDoc, source = 'rules', policySources = []) {
    ...
    if (projectedRevenueCollateral) {
        ...
        if (!['MNT', 'UNKNOWN'].includes(projectedCurrency) && !projectedRate) {
    ...
    try {
        ...
        const response = await Promise.race([
            openai.responses.create({
                ...
                temperature: 0,
                instructions: [
                    'Та Монголын ББСБ-ийн дотоод Зээлийн агент.',
                ...
                input: [{
                    role: 'user',
                    content: [
                        { type: 'input_text', text: `Зээлийн хүсэлтийн өгөгдөл:\n${JSON.stringify(input)}\n\nХавсаргасан журмын файлуудтай харьцуулж Зээлийн агентын дүгнэлт гарга.` },
                        ...blocks,
                    ],
                }],
                text: {
...
async function updateLoanOfficerAssessment(loanOrId) {
    ...
    await LoanRequest.findByIdAndUpdate(loan._id, {
        aiLoanOfficer: buildAiLoanOfficerStatus('running', 'Зээлийн агент дүгнэлт боловсруулж байна.'),
    });
...
function buildRuleBasedComplianceReview(loanDoc, policySources = [], source = 'rules') {
    ...
    const projectedRevenueCollateral = (input.collaterals || []).find(c => c.type === 'account_revenue');
    const projectedRevenueCurrency = String(projectedRevenueCollateral?.fields?.currency || 'UNKNOWN').toUpperCase();
    ...
    checks.push({
        ...
        policyClause: 'Харилцагчийг таньж мэдэх / бүртгэлийн баримт бүрдүүлэлт',
        ...
        conflictReason: missingDocuments.length
            ? 'Харилцагчийг таньж мэдэх / бүртгэлийн баримт бүрдүүлэлтийн шаардлагатай харьцуулахад хүсэлтийн бүрдэл дутуу тул тухайн заалттай бүрэн нийцэхгүй байна.'
            : 'Одоогийн бүрдэл нь харилцагчийг таньж мэдэх / бүртгэлийн баримт бүрдүүлэлтийн шаардлагатай нийцэж байна.',
        finding: missingDocuments.length ? 'Харилцагчийн баримтын бүрдэл журмын шаардлагатай бүрэн нийцэхгүй байна.' : 'Харилцагчийн үндсэн мэдээлэл журмын шаардлагатай нийцэж байна.',
        recommendation: missingDocuments.length
            ? 'Дутуу баримтыг нөхүүлж, тухайн журмын заалттай нийцсэн эсэхийг дахин шалгах.'
            : 'Иргэний/байгууллагын бүртгэлийн мэдээлэл болон хавсаргасан баримтыг эх хувьтай тулган баталгаажуулах.',
    });
    checks.push({
        area: 'Зээлийн судалгаа ба эргэн төлөлтийн чадвар',
        ...
        evidence: input.aiLoanOfficer?.status === 'completed'
            ? 'Зээлийн агентын урьдчилсан дүгнэлт бүртгэгдсэн байна.'
            : 'Зээлийн агентын дүгнэлт эсвэл бүрэн судалгааны баталгаажуулалт хараахан бүрдээгүй байна.',
        conflictReason: input.aiLoanOfficer?.status === 'completed'
            ? 'Зээлийн судалгаа / эргэн төлөлтийн чадварын үнэлгээний заалттай харьцуулахад Зээлийн агентын дүгнэлт бүртгэгдсэн боловч хүний ажилтны баталгаажуулалт шаардлагатай тул хэсэгчлэн нийцсэн, нягтлах шаардлагатай.'
            : 'Зээлийн судалгаа / эргэн төлөлтийн чадварын үнэлгээний заалттай харьцуулахад судалгааны нотолгоо бүрэн харагдахгүй тул мэдээлэл дутуу байна.',
        finding: input.aiLoanOfficer?.status === 'completed'
            ? 'Зээлийн агентын урьдчилсан дүгнэлт бүртгэгдсэн ч журмын шаардлагатай бүрэн нийцүүлэхийн тулд ажилтан баталгаажуулах шаардлагатай.'
            : 'Зээлийн судалгааны дүгнэлт бүрэн бүрдээгүй тул журмын шаардлагатай нийцээгүй/мэдээлэл дутуу байна.',
        recommendation: 'Орлого, өрийн ачаалал, барьцаа, зээлийн түүхийг тухайн заалтын дагуу ажилтан хянаж баталгаажуулах.',
    });
    ...
    properties: {
        ...
        checks: {
            ...
            items: {
                type: 'object',
                additionalProperties: false,
                required: ['area', 'status', 'severity', 'policyRef', 'policyClause', 'policyRequirement', 'evidence', 'conflictReason', 'finding', 'recommendation'],
                properties: {
                    area: { type: 'string' },
                    status: { type: 'string', enum: ['compliant', 'needs_review', 'non_compliant', 'insufficient_information'] },
        ...
        const response = await Promise.race([
            openai.responses.create({
                ...
                instructions: [
                    ...
                    'check бүр дээр policyRef, policyClause, policyRequirement, evidence, conflictReason талбаруудыг заавал бөглө.',
                    'policyClause талбарт боломжтой бол журмын заалтын дугаар, гарчиг, бүлэг, хэсэг эсвэл хуудасны лавлагааг бич. Заалтын дугаар тодорхойгүй бол хамгийн ойролцоо хэсгийн нэрийг бич.',
...
async function updateComplianceReview(loanOrId) {
    ...
    await LoanRequest.findByIdAndUpdate(loan._id, {
        complianceReview: buildComplianceStatus('running', 'Комплианс, хуулийн агент дүгнэлт боловсруулж байна.'),
    });
...
const generateLoanText = (borrower = {}, outputs = {}) => {
    ...
    const accountRevenue = safeList(outputs.collateral?.items)
        .find(item => item.collateralType === 'account_revenue' && item.projectedRevenueAnalysis);
    const revenuePlan = accountRevenue?.projectedRevenueAnalysis || {};
    const revenueCurrency = String(revenuePlan.currency || 'UNKNOWN').toUpperCase();
    const revenueRate = toNumber(accountRevenue?.projectedRevenueExchangeRate);
...
app.post('/api/loan-research', authenticateUser, (req, res) => {
    upload(req, res, async (err) => {
        ...
        try {
            ...
            const sourceRequestId = String(borrower.sourceRequestId || '');
            const projectedRevenueRows = safeList(borrower.collaterals)
                .filter(item => item.collateralType === 'account_revenue' && item.projectedRevenueAnalysis);
            if (sourceRequestId && projectedRevenueRows.length) {
                const linkedLoan = await LoanRequest.findById(sourceRequestId).lean();
                if (linkedLoan?.applicationData?.collaterals?.length) {
                    let projectedIndex = 0;
                    const applicationData = {
                        ...linkedLoan.applicationData,
                        collaterals: linkedLoan.applicationData.collaterals.map(collateral => {
                            if (collateral.type !== 'account_revenue') return collateral;
                            const researchCollateral = projectedRevenueRows[projectedIndex++];
                            if (!researchCollateral) return collateral;
                            return {
                                ...collateral,
                                fields: {
                                    ...(collateral.fields || {}),
                    ...
                    await LoanRequest.findByIdAndUpdate(sourceRequestId, { applicationData });
                    updateLoanOfficerAssessment(sourceRequestId)
                        .then(updated => updateComplianceReview(updated))
                        .catch(error => console.error('Research review refresh error:', error.message));
                }
            }
