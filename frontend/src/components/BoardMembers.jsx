...
const FALLBACK_BOARD = [
  { name: "Б.Дөлгөөн", role: "ТУЗ-ын дарга", experience: "Уул уурхай, олборлолтын салбарт 20 гаран жилийн ажлын туршлагатай.", imagePath: "/board/dulguun.jpg" },
  { name: "Б.Золбоо", role: "ТУЗ-ын гишүүн, Гүйцэтгэх захирал", experience: "Банк санхүүгийн салбарт 18 жилийн ажлын туршлагатай.", imagePath: "/board/zolboo.jpg" },
  { name: "Ц.Отгонбилэг", role: "ТУЗ-ын гишүүн, Ерөнхий захирал", experience: "Банк санхүүгийн салбарт 23 жилийн ажлын туршлагатай.", imagePath: "/board/otgonbileg.jpg" },
  { name: "В.Ганзориг", role: "ТУЗ-ын хараат бус гишүүн", experience: "Банк санхүү, маркетинг, медиа салбарт 22 жилийн туршлагатай.", imagePath: "/board/ganzorig.jpg" },
  { name: "Д.Энхтүвшин", role: "ТУЗ-ын нарийн бичгийн дарга", experience: "Банк санхүү, эм ханган нийлүүлэлтийн салбарт 18 жилийн туршлагатай.", imagePath: "/board/enkhtuvshin.jpg" }
];

const BoardMembers = () => {
  const [members, setMembers] = useState(FALLBACK_BOARD);

  useEffect(() => {
    axios.get(`${API_URL}/api/team/all`)
      .then(res => {
        const board = res.data.filter(m => m.memberType === 'board' && m.isActive !== false);
        if (board.length > 0) setMembers(board);
      })
      .catch(() => {});
  }, []);
          ...
          {members.map((member, index) => (
            <div key={index} className="flex flex-col items-center text-center w-full max-w-xs group animate-fade-in-up" style={{ animationDelay: `${index * 150}ms` }}>
              ...
              <div className="space-y-3">
                <h3 className="font-display font-bold text-2xl text-white group-hover:text-[#D4AF37] transition-colors duration-300 drop-shadow-md">{member.name}</h3>
                <p className="text-[#D4AF37] font-sans font-semibold text-sm uppercase tracking-wider px-3 py-1 rounded-full bg-[#003B5C]/60 border border-[#D4AF37]/30 inline-block">{member.role}</p>
                {member.experience && (
                  <p className="text-gray-300 text-sm leading-relaxed font-sans font-light opacity-90 max-w-[260px] mx-auto border-t border-white/10 pt-3 mt-3">{member.experience}</p>
                )}
              </div>
            </div>
          ))}
