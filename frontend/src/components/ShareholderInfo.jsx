...
const ShareholderInfo = () => {
  const [cfg, setCfg] = useState({
    shareholder_name: 'Б.Дөлгөөн',
    shareholder_percent: '100',
    shareholder_description: '"Солонго Капитал ББСБ" ХХК нь Монгол Улсын иргэний 100% өмчлөлд байдаг, дотоодын хөрөнгө оруулалттай компани юм.'
  });

  useEffect(() => {
    fetch(`${API_URL}/api/config/flat`)
      .then(r => r.json())
      .then(d => { if (d.shareholder_name) setCfg(prev => ({ ...prev, ...d })); })
      .catch(() => {});
  }, []);
    ...
    <section className="py-20 w-full flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="relative w-56 h-56 md:w-64 md:h-64 mb-12 group cursor-default z-10">
        ...
        <svg className="absolute inset-0 w-full h-full -rotate-90 drop-shadow-[0_0_20px_rgba(212,175,55,0.6)]" viewBox="0 0 100 100">
          <circle cx="50" cy="50" r="46" fill="transparent" stroke="#D4AF37" strokeWidth="6"
            strokeDasharray="289.02" strokeDashoffset="0" strokeLinecap="round" className="transition-all duration-1000 ease-out"/>
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center animate-fade-in">
          <span className="text-5xl md:text-6xl font-display font-bold text-white drop-shadow-lg tracking-tighter">
            {cfg.shareholder_percent}<span className="text-3xl text-[#D4AF37]">%</span>
          </span>
      ...
      <div className="max-w-4xl text-center space-y-8 animate-fade-in-up z-10 px-4">
        <div className="space-y-2">
          <h3 className="text-white/60 text-sm uppercase tracking-[0.2em] font-sans">Хувьцаа эзэмшигч</h3>
          <h1 className="text-4xl md:text-6xl font-display font-bold text-white leading-tight">
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] via-[#F4E285] to-[#D4AF37]">{cfg.shareholder_name}</span>
          </h1>
        </div>
