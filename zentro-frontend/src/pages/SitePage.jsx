  ...
  return (
    <div className={`zp-site ${editor ? 'is-editor' : ''} is-${viewport}`} style={themeStyle}>
      ...
      <footer className="zp-footer" id="contact">
        ...
        <div className="zp-footer-bottom zp-container"><span>© {new Date().getFullYear()} {config.brandName}</span><EditableText as="span" path="siteContent.footerLegal" value={content.footerLegal} config={config} editor={editor} selection={selection} onSelect={onSelect} onChange={onChange} /><div className="zp-footer-legal-links"><a href="/privacy" onClick={event => { if (editor) event.preventDefault(); }}>Нууцлал</a><a href="/data-deletion" onClick={event => { if (editor) event.preventDefault(); }}>Мэдээлэл устгах</a></div></div>
      </footer>
